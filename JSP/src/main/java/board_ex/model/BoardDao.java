...
public class BoardDao
{
	...
	*/
	public String insert( BoardVO rec ) throws BoardException
	{
		...
		try{
			...
			//* sql 문장 만들기
			String putQuery	= " INSERT INTO board_ex(seq, regdate, cnt, writer, title, content, pass) "
					+ " VALUES(seq_board.nextval, sysdate, 0, ?, ?, ?, ?) ";  
			String seqSelectQuery = "SELECT seq_board.currval seq FROM dual";
			

			ps1 = con.prepareStatement( putQuery );
			ps2 = con.prepareStatement( seqSelectQuery );
			//* sql 문장의 ? 지정하기
					
			ps1.setString(1, rec.getWriter());
			ps1.setString(2, rec.getTitle());
			ps1.setString(3, rec.getContent());
			ps1.setString(4, rec.getPass());
			...
			if( ps1   != null ) { try{ ps1.close();  } catch(SQLException ex){} }
	...
	*/
	...
	public List<BoardVO> selectList() throws BoardException
	{
		...
		try{
			...
			// * sql 문장만들기
			String sql = " SELECT seq, title, writer, regdate, cnt FROM board_ex ORDER BY seq DESC ";
			// * 전송객체 얻어오기
			...
			throw new BoardException("게시판 ) 전체 레코드 검색시 오류  : " + ex.toString() );	
		} finally{
			if( rs   != null ) { try{ rs.close();  } catch(SQLException ex){} }
	...
	*/
	...
	public List<BoardVO> selectList(int firstRow, int endRow) throws BoardException
	{
		...
		try{
			...
			throw new BoardException("게시판 ) 한 페이지의 레코드를 검색시 오류  : " + ex.toString() );	
		} finally{
			if( rs   != null ) { try{ rs.close();  } catch(SQLException ex){} }
			if( ps   != null ) { try{ ps.close();  } catch(SQLException ex){} }
			if( con  != null ) { try{ con.close(); } catch(SQLException ex){} }
	...
	*/
	 ...
	 * 리턴값 :   총 레코드 수 리턴
	*/
	public int totalRecCount() throws BoardException {
		PreparedStatement ps = null;
		ResultSet rs = null;
		int count = 0;
		...
		try{
			...
			throw new BoardException("게시판 ) 총 레코드 수 검색 오류  : " + ex.toString() );	
		} finally{
			if( ps   != null ) { try{ ps.close();  } catch(SQLException ex){} }
			if( con  != null ) { try{ con.close(); } catch(SQLException ex){} }
		}

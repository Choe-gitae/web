...
<%
	// 한 페이지 레코드 검색
	List <BoardVO> mList =  ListArticleService.getInstance().getArticleList(pageNum);
...
<BODY>
	...
	<div>
		...
		<a href="BoardList.jsp?startPage=<%=startPage%>&prevBtn=on"><button>이전</button></a>
		<% for(int i=startPage; i<=endPage; i++) { %>
		<a href="BoardList.jsp?pageNum=<%=i%>&startPage=<%=startPage%>">[ <%=i%> ]</a>
		<% } %>
		<a href="BoardList.jsp?startPage=<%=startPage%>&nextBtn=on"><button>다음</button></a>
		<a href=""><button>마지막</button></a>
	</div>
</BODY>
</HTML>
